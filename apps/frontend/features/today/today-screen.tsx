...
import {
  ...
  ListChecks, Play, RefreshCw, Search, ShieldCheck, Target, X,
} from "lucide-react";
...
export function TodayScreen() {
  ...
  return <>
    ...
    {review && <div className="modal-wrap" onClick={() => setReview(false)}><div className="review-modal" onClick={(event) => event.stopPropagation()}><button className="close icon-button" onClick={() => setReview(false)} aria-label="Закрыть" type="button"><X /></button><span className="round-icon blue"><RefreshCw /></span><p className="accent-label">Повторение · вопрос 1 из 4</p><h2>Объясни своими словами: почему собственный вектор не меняет направление при линейном преобразовании?</h2><textarea placeholder="Сформулируй ответ без подсказки…" autoFocus /><div className="modal-actions"><button className="secondary" onClick={() => setReview(false)} type="button">Отложить</button><button className="primary" type="button">Проверить ответ <ArrowRight /></button></div></div></div>}
  </>;
}
